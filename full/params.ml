...
module L = struct
  ...
  let sort = makefile % create (fun x-> x.sort) (fun x y -> { x with sort = y })
  ...
  let no_std_otherlibs = analyzer % create (fun x-> x.no_std_otherlibs)
      (fun x y -> { x with no_std_otherlibs = y })

...
let round_trip sch x =
  incr counter;
  let print scheme ppf x = Scheme.minify ppf "%a" (scheme @@ wrap sch) x in
  let s scheme = Format.asprintf "%a" (print scheme) x in
  let map2 f (x,y) = (f x, f y) in
  let j, s = map2 s Scheme.(json,sexp) in
  Format.printf "@[%d)@ " !counter;
  let check name s = match Scheme.retype sch (parse s) with
  | Some y ->
    if x = y then
...
let round_trip' sch src trip =
  ...
  | _ :: (next :: _ as rest) ->
      ...
      | Some x ->
        let print ppf x = Scheme.minify ppf "%a" (next @@ wrap sch) x in
